...
import { HistoryAnalytics } from "@/app/pomodoro/history/_components/history-analytics";
import { HistoryDateBrowser } from "@/app/pomodoro/history/_components/history-date-browser";
import { HistoryExportButton } from "@/app/pomodoro/history/_components/history-export-button";
import { HistoryOverview } from "@/app/pomodoro/history/_components/history-overview";
import { HistorySessionList } from "@/app/pomodoro/history/_components/history-session-list";
import { HistoryTagSummary } from "@/app/pomodoro/history/_components/history-tag-summary";
...
export function HistoryView({
  ...
  const exportHref = useMemo(() => {
    ...
    return queryString.length > 0
      ? `/pomodoro/api/history/export?${queryString}`
      : "/pomodoro/api/history/export";
  }, [deferredTagQuery, modeFilter, periodFilter, selectedDate]);
  ...
  async function handleDelete(sessionId: string) {
    ...
    try {
      const response = await fetch(
        `/pomodoro/api/sessions?id=${encodeURIComponent(sessionId)}`,
        {
          method: "DELETE",
        },
      );
